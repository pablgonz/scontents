...
%%                                 scontents-code.tex.
%% 
\def\ScontentsCoreFileDate{2019-12-25}%
\begingroup
  \def\next{\endgroup}%
...
\cs_new_protected:Npn \__scontents_scontents_setenv:nnn #1 #2 #3
  {
    \cs_new_protected:cpn { __scontents_#2_env_begin: }
      {
        \tl_set:Nn \l__scontents_name_seq_env_tl {#2}
        \tl_set:Nn \l__scontents_env_name_tl {#1}
        \keys_set:nn { scontents } {#3}
        \__scontents_setup_verb_processor:
        \__scontents_env_generic_begin:
      }
    \cs_new_protected:cpn { __scontents_#2_env_end: }
      { \__scontents_env_generic_end: }
    \exp_args:Nooo % http://nooooooooooooooo.com
    \__scontents_env_define:nnn { \tl_to_str:n {#1} }
      { \cs:w __scontents_#2_env_begin: \cs_end: }
      { \cs:w __scontents_#2_env_end: \cs_end: }
  }
      ...
      \tl_if_blank:nF {#2}
        {
          \msg_error:nnxn { scontents } { junk-after-begin }
            { after~\c_backslash_str begin { \tl_use:N \l__scontents_env_name_tl } #1 } {#2}
        }
    ...
    \use:x
      {
        \cs_set_protected:Npn \exp_not:N \__scontents_analyse_nesting_generic:w ####1
          \c_backslash_str #1 \tl_use:N \l__scontents_env_name_tl ####2 \exp_not:N \q__scontents_stop
